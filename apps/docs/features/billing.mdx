...
boundary is provider-interface based, so a self-hosted team can add another provider adapter without
rewriting the product request path or moving pricing logic back into application code.
