...
Use `@unprice/api` in your server request path to authorize customer spend before paid work runs.
Create or map a customer, check access, consume usage, reserve a workload budget, and inspect usage
or wallet evidence.
...
For variable provider usage — an LLM call whose cost is not known until it returns — hold a
maximum before the work and settle what it actually cost after:
...
`reservations` is a helper over the `runs` operations, not a separate API. Read
[the reservations helper](/libraries/ts/sdk/runs/reservations) for what each call maps to, and use
`runs.start` / `runs.consume` / `runs.settle` / `runs.end` directly for multi-step workloads.
