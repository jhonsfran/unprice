...
Use `runs.consume` before a billable step when its usage amount is known. It applies a synchronous
metered event to the run and can reject the step before it creates cost. If the provider has already
incurred variable usage, use [runs.settle](/libraries/ts/sdk/runs/settle).

<RequestExample>

```ts
// Authorize known usage before starting the billable step.
const { result, error } = await unprice.runs.consume({
  runId: "brun_123",
  featureSlug: "ai-tools",
  eventSlug: "completions",
  idempotencyKey: "run:req_123:step_1",
  // This call consumes only the selected feature.
  properties: {
    aiTools: 3,
    step: "summarize"
  }
