...
For example `GET https://api.unprice.dev/v1/plans/versions/get/{planVersionId}` to get the configuration of a plan.
...
```http
curl "https://api.unprice.dev/v1/plans/versions/get/plan_123"
...
```http
curl -XPOST "https://api.unprice.dev/v1/plans/versions/list"
  -H "Authorization: Bearer <API_KEY>"
