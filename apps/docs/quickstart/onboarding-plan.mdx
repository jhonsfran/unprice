---
title: "One agent run"
description: "Put one agent run behind an authorization."
---

Start with the action in your product that can spend against a customer's budget, credits, or
usage allowance. Prove that one path before you migrate billing or enforce every feature.

The first integration has one job:

> Authorize one run before it spends, then keep the receipt that explains what happened.

## 1. Pick the action

Start with an agent run. The same pattern covers any paid action — pick a narrow one with a clear
