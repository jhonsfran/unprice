...
> `runs` hold a budget before a workload starts, for cost that is not known yet.
> `access.check` asks without changing state.
...
| Hold a maximum for one variable-cost call | `reservations.reserve` then `reservation.settle` | Yes | Yes | LLM calls priced on tokens you cannot predict |
| Authorize spend across multi-step work | `runs.start` / `runs.consume` / `runs.settle` / `runs.end` | Yes | Yes | Workflows, jobs, tools, agents, custom workloads |
...
[reservations helper](/libraries/ts/sdk/runs/reservations) wraps the same operations in three
