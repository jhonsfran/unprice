...
> Name the one paid action in your product that can burn margin or create invoice confusion. Define
> one plan version, and a single `signUp` call provisions the customer, subscription, entitlements,
> credit grants, billing periods, and wallet for you; then Unprice authorizes that action in the
> request path, reserves or captures credits when needed, and makes every allow, deny, charge,
> credit, and invoice line explainable — testable end-to-end on a built-in sandbox before a real
> dollar moves, on an open-source core you can read before you trust it.

---

## The Offer Name (MAGIC Formula)

**Primary:** **The Paid-Action Authorization Sprint**
...
| --- | --- | --- |
| **Dream Outcome** | Maximize | Reframe from "better invoices" to **commercial control with proof**: *"Your paid action is authorized before it runs, and every invoice line can prove why it exists."* The win is both a blocked over-budget request and a charge support can explain without reconstructing logs. |
| **Perceived Likelihood** | Maximize | Shrink the promise to one plan version, one paid action, one authorization path, one evidence trail. Show the exact SDK methods that do it (`customers.signUp`, `access.check`, `usage.consume`, `runs.start` / `runs.consume` / `runs.end`, `wallet.balance`, invoice explanation). Define the plan once and `signUp` provisions the subscription + entitlements for you. The core is **open source** — they can read the money-path code before trusting it. Start small, prove it, expand. |
| **Time Delay** | Minimize | **No full billing migration, and no real processor to start.** Define one plan version, call `signUp` (it wires the subscription + entitlements), then authorize one action with `access.check` / `usage.consume` — runnable on the built-in **Sandbox** provider before a dollar moves. First request path live in a sprint, not a roadmap quarter. |
...
The strategic result: the *value* (authorized usage + protected margin + eliminated DIY build +
explainable invoices) is large and the *price* (an afternoon defining one plan version and wiring
two SDK calls into one request path) is small. That gap is the offer.
...
1. **Define one plan version, once (dashboard).** Create the feature(s) you want to gate and publish
   a plan version that includes them. The plan version is the entitlement map for assigned customers
   — `access.check` later matches on the feature slug you define here. (Plans are authored in the
   dashboard/tRPC, not the runtime SDK.)
...
3. **Authorize the paid action.** `access.check({ customerId, featureSlug })` returns allow/deny in
   the request path; for usage features, report with `usage.record` / `usage.consume`, and cap
   workloads with budgeted runs (`runs.start` / `consume` / `end`). The same path preserves evidence
   for the invoice explanation later.
...
| --- | --- | --- | --- |
| 1 | **Commercial authorization for one paid action** — check entitlement, budget, credits, and meter rules in the request path before work runs. | Let the action run first and reconcile later. | Margin leaks, customer disputes, and support escalations after usage already happened. |
| 2 | **Explainable invoice evidence** — every charge traces back to the plan version, rated usage events, wallet movement, and ledger captures. | Reconstruct a disputed line by hand from logs. | Senior-eng + support hours per dispute; eroded trust. |
| 3 | **Auto-provisioned entitlements + runtime metering** — `signUp` derives entitlements from the plan version; check access and consume usage while the request is in flight. | Hand-build a subscriptions + entitlements table and scatter `if (plan === ...)` checks through product code. | Every packaging change becomes a code change + redeploy. |
...
- **Bonus 1 — The Paid-Action Map.** A short worksheet to find the product action that creates cost
...
- **Bonus 4 — The Inspectable Core.** The AGPL-3.0 source for the money path itself — read every line
  ...
  objection.*
- **Bonus 5 — Plan Versioning Without Surprise Migration.** Version plans and keep existing
...
> budget, wallet, evidence — before you migrate anything else. If that single path does not make the
> decision clearer and safer than your current counter, cron job, or billing script, you should know
> **before** you move the rest of your stack. We will tell you if it is not a fit.
...
> full money path — customers, plan versions, entitlements, budgets, credits, invoices — on the built-in
...
> today; the invoice that explains it is weeks away. The gap between those two is the leak.
...
> call, the data job, the costly API? Right now you find out at invoice time, when the work already
> ran. Unprice authorizes that action before it runs and keeps the evidence that proves every charge.
> Open source, one request path to start, tested on a sandbox before any real money moves. Want to
> map yours?
...
> disputes it, an engineer reconstructs the path by hand.
>
...
1. **Authorize paid usage** (primary — broad, on-brand)
2. **Budget my expensive action** (campaign / high-cost use case)
3. **Start with one paid action** (low-commitment first step)
4. **Explore the request-path SDK** (developer secondary)
5. **Star on GitHub** (community / trust secondary)
