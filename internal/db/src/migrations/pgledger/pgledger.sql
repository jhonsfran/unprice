-- Vendored from https://github.com/pgr0ss/pgledger.
-- See ./VERSION and ./README.md for the pinned commit hash and attribution.
-- Modified to be idempotent: tables use IF NOT EXISTS, functions use OR REPLACE,
-- views use OR REPLACE, indexes are named and use IF NOT EXISTS, and the
-- composite TRANSFER_REQUEST type is created defensively via DO/EXCEPTION.
...
-- Define a composite type for transfer requests (idempotent: only create when missing)
DO $$
BEGIN
