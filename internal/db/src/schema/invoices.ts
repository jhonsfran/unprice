...
export const invoiceItems = pgTableProject(
  "invoice_items",
  {
    ...
    // Traceability pointer to the pgledger transfer that originated this line item.
    // pgledger lives outside Drizzle's schema, so this is text (transfer id strings
    // like `pglt_<ulid>`) rather than a Drizzle foreign key.
