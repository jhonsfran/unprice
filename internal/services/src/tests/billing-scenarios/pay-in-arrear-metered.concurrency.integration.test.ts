...
async function runBillPeriodWorker(dbForWorker: Database) {
...
describe("P0-A pay_in_arrear metered billing concurrency", () => {
  afterAll(async () => {
  ...
  it("serializes concurrent billing for one statement into one invoice and one ledger posting", async () => {
    const workerDbs = Array.from({ length: 4 }, () => createTestDatabaseConnection())

    try {
      const results = await Promise.allSettled(
        workerDbs.map((workerDb) => runBillPeriodWorker(workerDb))
      )

      expect(results).toHaveLength(workerDbs.length)
      ...
      `)
      expect(idempotencyCount.rows).toEqual([{ count: 2 }])
