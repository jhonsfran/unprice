...
describe("P0-D pay_in_advance capped wallet workflow", () => {
  afterAll(async () => {
  ...
  it("P0-D invoices fixed charges at period start and gates usage with a capped wallet runway", async () => {
    ...
    const reservation = await wallet.createReservation({
      ...
      idempotencyKey: "reserve:p0-d-2026-01:events",
    })
    ...
    await expectWalletState(wallet, {
      consumed: fixedAmount + usageAmount,
      creditCount: 0,
      granted: 0,
      reserved: 0,
    })
